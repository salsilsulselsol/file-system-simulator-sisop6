...
public class SuperBlock {
    ...
    // PERBAIKAN: Mengubah tipe kembalian dari long menjadi int
    public int getDataBlockBitmapOffset() {
    ...
    private void calculateOffsets() {
        ...
        int maxFileCount = Math.max(100, (int) Math.ceil(dataBlockCount / 10.0));
        inodeCount = maxFileCount;

        short superBlockCount = 1;
        int inodeBitmapBlockCount = (int) Math.ceil((double) inodeCount / (blockSize * 8.0));
        int dataBitmapBlockCount = (int) Math.ceil((double) dataBlockCount / (blockSize * 8.0));
        int inodeBlockCount = (int) Math.ceil((double) (inodeCount * IndexNode.INODE_SIZE) / blockSize);
